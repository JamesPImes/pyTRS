...
#######################################################################
#
# <regexLib> -- Regex Library
#
#######################################################################

# Township & Range + Section regexes...

# Should find township & ranges, formatted with up to 3 digits for twp and for rge
# ('000n000w' or '0n0w' or between).
# Note that this is somewhat more complicated than it /should/ be because a Range of '2'
# can accidentally capture as a Range number in, for example "N2N2, W2E2" (i.e. could
# match as 'T2N-R2W'). So we disallow specifically a singular '2' when 'R' (or the word
# 'Range') does not preceed it.
...
# A singular '2' will show up as .group(12); and direction is .group(13). Otherwise, Range
# number is .group(6), and direction is .group(8)

# A more broadly-capturing regex for our standard T&R format (this one does not require
# 'T' or 'R', but also wildly captures "Lot 2, N2 W2" as <'T2N-R2W'>. Should not be used
# for unprocessed text.)
...
# This should find sections formatted with up to 3 digits (e.g., '013', '009', etc. should
# be captured):
...
# This should find multiple sections, formatted up to 3 digits (e.g., "Sections 5, 6,
# 8 - 12"). Unpack with: unpack_sections()
# Note that this will also match a single section, so individual sections must be ruled
# out (i.e. individual section matches will have mo.group(12) != None). Can use funcs
# is_multisec() and is_singlesec() on the match objects.  This regex will also capture an
...
# A regex that will match 'Section' (or equivalent abbreviation / typo), even when there's
# no number:
...
# require T and R, but not n/s or e/w  (will need to fill in with kwarg-specified default):
...
# With enough context, will capture T&R's with OCR artifacts (e.g. "TIS4N-R97W" instead of
# intended "T154N-R97W").  Gets converted to numerics in preprocess_tr_mo().
...
# A regex for extra context around TR_noR_noNS (need to rule out "Lots" at the start of
# such a match):
...
# cleanQQ regexes, for parsing aliquots under cleanQQ=True conditions. Will match much
# more broadly.
...
# N2, S2, E2, and W2 are the same under cleanQQ conditions, since there still must be SOME
# designator that it's a 'half'.

# 'E2NE' should be enough context to interpret it as the E½NE¼. This regex can be applied
# after subbing the other 'half' regexes in for their cleaner counterparts, so
# 'E2NE' -> 'E½NE' --> 'E½NE¼'. Must be at word boundaries, per \b.
...
# Lot regex (should capture entire match, but then requires some processing to unpack it):
...
#     group(15) is the connector between the last lot and the lot before it, possibly with
#         superfluous text / characters ('and' or 'through' -- or equivalent symbol / abbrev)
#     group(14) ends with the stated acreage of the last lot ONLY when it occurs at the
#         end of the matched string.  Get that stated acreage with lotAcres_unpacker_regex,
#         which is baked into the get_lot_acres() function.
...
lot_with_aliquot_regex = re.compile(
    ...
    """, re.IGNORECASE | re.VERBOSE)
# Guide to lot_with_aliquot_regex groups (using "N½N½ of Lots 1 - 3" as the example):
# group(1) is the entire aliquot component + ' of ' -- i.e. 'N½N½ of ' in the example;
# group(2) is the entire aliquot component without the 'of' -- i.e. 'N½N½' in the example;
# group(7) is the entire lots component -- i.e. 'Lots 1 - 3' in the example
...
# The last group is always the dominant (right-most) division (may or may not have the
# fraction attached). Everything to the left of that needs to be recursively processed.

# For cutting out whitespace and 'of the' or 'of' between identified aliquot components:
aliquot_intervener_remover_regex = re.compile(
    ...
    """, re.IGNORECASE | re.VERBOSE)
### group(1) is the first aliquot component -- i.e. 'N½' in the example "N½ of the NE¼";
### group(8) is the second aliquot component -- i.e. 'NE¼' in the example "N½ of the NE¼".


# For cutting out whitespace and 'of the' or 'of' between an aliquot and lot (for handling
# 'N/2 of Lot 1'):
...
# Looking for the phrase (or abbreviation for) 'Principal Meridian' and following symbols
# and whitespace. Not the cleanest definition, but will only be used in limited places
# (between a twprge_regex match and a sec_regex match), so it need not be too tight:
pm_regex = re.compile(
    r"""(P\.?\s{0,10}M\.?|
...
# When the section number(s) occur /within/ the description block (e.g., 'That portion of
# Section 14 lying west of RR')
