...
1) `Tract` attributes -- i.e. parsed data (see [the table in this guide](tract_attributes.md#tract-attributes))

2) Extracting data from `Tract` objects in bulk (see [this other guide](extracting_data.md))

3) `Tract` objects [created by `PLSSDesc` objects](#from-plssdesc) (i.e. parsing a raw PLSS land description)

4) `Tract` objects [created directly](#create-directly).

5) Populating the [lists of lots and aliquots](#parsing).


## `Tract` attributes

See [the table here](tract_attributes.md#tract-attributes) for all `Tract` attributes -- i.e. the meaningful data generated by the parsers.


## <a name='extracting'>Extracting data in bulk from `Tract` objects</a>

[See the guide on extracting `Tract` data in bulk.](extracting_data.md)


## <a name='from-plssdesc>`Tract` objects created from parsing a `PLSSDesc`</a>
...
## <a name='create-directly'>Creating `Tract` objects directly</a>

When creating a `Tract` object directly, specifying the `trs` is optional; and populating `lots`, `qqs`, `lots_qqs`, and `lot_acres` attributes is also optional (parsing is turned on with `parse_qq=True` and [optionally configured with `config=`](config.md)).
```
...
|`trs=<str>`            | The Twp/Rge/Sec in the [standard pyTRS format](trs.md#standard)                         | 1       |
|`config=<str>`         | [Configure the parser with a number of options](config.md)                        | 2       |
...
*[See `.from_twprgesec()` footnotes 1 - 4, which are identical.](#from_twprgesec-footnotes)*


#### <a name='from_twprgesec'>Create `Tract` objects with uncompiled Twp/Rge/Sec components with `Tract.from_twprgesec()`</a>
...
##### <a name='from_twprgesec-footnotes'>`Tract.from_twprgesec()` Footnotes:</a>
...
*__Note:__* Control the size of the parsed aliquots with [`config=` parameters](config.md), specifically the [`qq_depth_min`, `qq_depth_max`, or `qq_depth` parameters](config.md#depth). By default, they are parsed into quarter-quarters (QQs), i.e. 40-acre divisions (e.g., `'W/2NE/4' -> ['NWNE', 'SWNE']`) -- but will also include smaller aliquot divisions if they exist in the description (e.g., `'E/2W/2NE/4' -> ['E2NWNE','E2SWNE']`).


### <a name='setting-trs'>Setting Twp/Rge/Sec (`.trs` attribute, etc.)</a>

Setting the `.trs` attribute of a `Tract` object will populate the other associated attributes accordingly (`.twp`, `.rge`, `.sec`, etc.) -- and it can be done either when created (`a_tract = pytrs.Tract("NE/4", trs='154n97w14')` or with [`Tract.from_twprgesec()`](#from_twprgesec), assigned directly afterwards (`a_tract.trs = '154n97w14'`) or with the `.set_twprgesec()` method:
```
