...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-i', '--input_csv', type=str, help='path to input .csv file',
        required=True)
    parser.add_argument(
        '-o', '--output_csv', type=str, help='path to new output .csv file')
    parser.add_argument(
        '-dc', '--desc_column', type=int,
        help=(
            'The number of the column (indexed from 1) '
            'containing PLSS descriptions'
        ),
        required=True)
    parser.add_argument(
        '-a', '--attribs', type=str,
    ...
    # A negative `header_row` means there are no headers in the input
    # (or that headers won't be handled as such):
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "The number of the column (indexed from 1) containing the "
            "layout for each description (requires the user to prep "
            "input .csv to specify the layout for each row)"
        ))
    parser.add_argument(
        '-cc', '--config_column', type=int,
        help=(
            "The number of the column (indexed from 1) containing the "
            "config parameters for each description (requires the user "
            "to prep input .csv to specify config parameters for each "
            "row)"
        ))
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Tracts have already been separated from TRS in input .csv; "
            "now only parsing lots and QQ's"
        ))
    ...
    parser.add_argument(
        ...
        help=(
            "unpack any Tract data stored as a list when writing to "
            "output .csv file"
        ))
    parser.add_argument(
        '-uid', '--include_uid', action='store_true',
        help=(
            "Generate unique identifier number (UID) for each "
            "description parsed"
        ))
    parser.add_argument(
        '-iu', '--include_unparsed', action='store_true',
        help=(
            "Also write the unparsed rows (i.e. copy them) to the "
            "output .csv file"
        ))
    parser.add_argument(
        '-nt', '--number_tracts', action='store_true',
        help=(
            "Generate a separate .csv file that states how many rows "
            "were written for each description"
        ))
    parser.add_argument(
    ...
    # `parse_csv()` is capable of creating a default output filepath. It
    # has also been implemented here to allow us to optionally launch
    # the file after parsed.
    out_filepath = args['output_csv']

    out_filepath = parse_csv(
